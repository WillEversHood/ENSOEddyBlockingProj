...
from Blocking_Energy import
import seaborn as sns
import pandas as pd
import sympy as sp
# blocking energy has not yet been tested or debugged


'''
Model is constructed with the fundamental assumption that a value will be solved for 30X per month. 
Thus, when selecting the resolution make certain that for each time increment(1month)
the resolution parameter is time * 30

working on a permanent fix such that this is not something that can be messed up by human mistake
'''

class Coupled():
    def __init__(self, aRD, a84, bRD, b84, c, T, h, F_0_RD, F_0_84, F_1_84, G_0_84, G_1_84, a_nx, a_ny, X, Y, Z, t):
        self.aRD = aRD
        self.a84 = a84
        self.bRD = bRD
        self.b84 = b84
        ...
        self.reso = t * 30 # number of locations that will be
    ...
    def NLLE (self, vector, runout, visualization):
        '''
        ...
        '''
        Issue arises when resolution/months < 1
        '''
        ...
        if visualization == True:
            ...
            plt.plot(months, final_lles, label='Monthly LLE values', color='b', marker='o')
...
biggin = Coupled(-0.076,0.25,0.236, 4 , -0.125,1 , 1,0.17,7, 1.5625,0,1.5625,0.00513 , -0.184, 1, 1, 1, 30, 900)
...
#biggin.JPDF(vector)
#predictability = biggin.NLLE(vector, 3, True)
